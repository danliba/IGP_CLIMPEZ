...
data1<-read_excel("variables_1.xlsx", sheet = "all_Data")
View(data1)
data_fisica<-data1[,2:10]
View(data_fisica)

row.names(data_fisica)<-c(2000:2019)

windows()
par(mfrow=c(3,3))

#plot(density.default(aa),main="depth");shapiro.test(data_pc[,1]) #pval >0.05
plot(density.default(scale(data_fisica[,1])),main="PPmensualCuenca");shapiro.test(data_fisica$PPmensualCuenca) #pval >0.05 / normal
plot(density.default(scale(data_fisica[,2])),main="TempDia");shapiro.test(data_fisica$TempDía) #pval >0.05 / normal 
plot(density.default(scale(data_fisica[,3])),main="TempNoche");shapiro.test(data_fisica$TempNoche) #pval >0.05 /normal
plot(density.default(scale(data_fisica[,4])),main="SedimentosRequena");shapiro.test(data_fisica$SedimentosRequena) #pval >0.05 / normal
plot(density.default(scale(data_fisica[4:16,5])),main="SedimentosChazuta");shapiro.test(data_fisica$SedimentosChazuta) #pval >0.05 /normal
plot(density.default(scale(data_fisica[1:18,6])),main="SedimentosTamshiyacu");shapiro.test(data_fisica$SedimentosTamshiyacu) #pval > 0.05 /normal
plot(density.default(scale(data_fisica[,7])),main="CaudalTamshiyacu");shapiro.test(data_fisica$CaudalTamshiyacu) #pval < 0.05 / no es normal
...
plot(density.default(scale(data_fisica[1:19,9])),main="CaudalChazuta");shapiro.test(data_fisica$CaudalChazuta) #pval < 0.05

%>% ################################################################################
######################## Variables biologicas ##################################
################################################################################

data_bio<-data1[,11:25]
View(data_bio)
row.names(data_bio)<-c(2000:2019)
...
plot(density.default(scale(data_bio[1:18,11])),main="Yu-Paiche");shapiro.test(data_bio$`Yu-Paiche`) #pval > 0.05 /  normal
...
data0<-read_excel("TOTAL_PCA.xlsx", sheet = "PCA_anual")
View(data0)
data1<-data0[,4:13]
View(data1)
